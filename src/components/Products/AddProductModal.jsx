...
export default function AddProductModal({ isOpen, onClose, onSuccess }) {
  ...
  const { register, handleSubmit, watch, formState:{ errors, isSubmitting }, reset, setValue } = useForm({
    ...
    defaultValues: { companyId:'', categoryId:'', name:'', sku:'', defaultPrice:'', shelfLifeDays:'' },
  });
  ...
  useEffect(() => {
    ...
    reset({ companyId:'', categoryId:'', name:'', sku:'', defaultPrice:'', shelfLifeDays:'' });
    if (user.role !== 'SUPER_ADMIN' && user.companyId) setValue('companyId', user.companyId);
  }, [isOpen, reset, setValue, user]);

  // Load categories when company is selected
  useEffect(() => {
    if (selectedCompany) {
